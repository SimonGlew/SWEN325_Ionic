...
export class HomePage {
  currentDate: any
  dayEvents: Array<{ id: any, time: number, readableTime: string, event: any }>;
    ...
    this.events.subscribe("events:getAll", (events) => {
      if (events && events.length) {
        events.forEach(e => {
          ...
          this.dayEvents[event.time] = { id: e.id, time: event.time, readableTime: this._formatTime(event.time), event: event.event }
        })
      }else{
        this.remakeDayEventsArray()
      }
  ...
  remakeDayEventsArray() {
    this.dayEvents = []
    for (let i = 0; i < 24; i++) {
      this.dayEvents.push({ id: null, time: i, readableTime: this._formatTime(i), event: '' })
    }
  }
